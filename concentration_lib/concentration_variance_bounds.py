...
def chi2_std_dev_bound(
    ...
    **kwargs,
    ...
    if side == "lower":
        ...
        bound_mean = -sigma_hat + sigma_hat * np.sqrt((n - 1) / chi2(n - 1).ppf(delta))
    else:
        bound_mean = np.array(
            [
                sigma_hat - sigma_hat * np.sqrt((n - 1) / chi2(n - 1).ppf(1 - delta)),
                -sigma_hat + sigma_hat * np.sqrt((n - 1) / chi2(n - 1).ppf(delta)),
            ]
        )
...
def hoeffding_std_dev_bound(
    ...
    **kwargs,
    ...
    bound_mean = (
        ...
        / 2
        * (2 / np.floor(n / 2) * np.log(1 / delta)) ** 0.25
    )
...
def maurer_pontil_std_dev_bound(
    ...
    **kwargs,
    ...
    bound_mean = (upper_bound - lower_bound) * np.sqrt(2 / (n - 1) * np.log(1 / delta))
    return return_bound(n, bound_mean, side, "mean", mode)
...
def bentkus_pinelis_std_dev_bound(
    ...
    **kwargs,
    ...
    q = (upper_bound - lower_bound) * norm.ppf(1 - delta / c) / (2 * np.sqrt(2 * n2))
    if side == "lower":
...
def chi2_zero_mean_std_dev_bound(
    ...
    **kwargs,
    ...
    if side == "lower":
        bound_mean = sigma_hat - np.sqrt(S2 / (chi2(n).ppf(1 - delta) / n))
    elif side == "upper":
        bound_mean = -sigma_hat + np.sqrt(S2 / (chi2(n).ppf(delta) / n))
    else:
        bound_mean = np.array(
            [
                sigma_hat - np.sqrt(S2 / (chi2(n).ppf(1 - delta) / n)),
                -sigma_hat + np.sqrt(S2 / (chi2(n).ppf(delta) / n)),
            ]
        )
